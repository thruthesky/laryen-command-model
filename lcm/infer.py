...
def split_compound(text: str):
    """다중동작이면 [첫 명령, 나머지], 아니면 None. *가장 앞* 연결어미에서 분할하고
    연결어미를 종결어미로 복원한다(가장 앞이어야 first 가 단일 — 3+ action 재귀 분할)."""
    best = None  # (index, conn, restore)
...
class LcmRuntime:
    ...
    def classify(self, text: str) -> dict:
        ...
        if parts:
            ...
            first = self._classify_one(parts[0])
            rest = self.classify(parts[1])
            if first["layer"] == "sml" and rest["layer"] == "sml":
                actions = [first["intent"]] + rest["command"]["actions"]
                return {"layer": "sml",
                        "confidence": min(first["confidence"], rest["confidence"]),
                        "command": {"actions": actions, "say": ""}}
            return {"layer": "fallback", "confidence": 0.0, "intent": None}
